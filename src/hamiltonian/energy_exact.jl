...
function energy_exact(h::Hamiltonian, nnqs::AbstractNNQS)
	L = sys_size(nnqs)
	E_loc = zero(local_energy(h, nnqs, ones(Int, L)))
	indices = CartesianIndices(ntuple(x->2, L))
	p_all = 0.
	for index in indices
		state = index_to_state(index)
		E_loc_single = local_energy(h, nnqs, state)
		amp = Ψ(nnqs, state)
		p_single = abs2(amp)
		p_all += p_single
...
function energy_and_grad_sr_exact(h::Hamiltonian, nnqs::AbstractNNQS; diag_shift::Real=1.0e-2, λ::Real = 1.0e-6)
	L = sys_size(nnqs)
	E_loc, ∂θ = local_energy_and_grad(h, nnqs, ones(Int, L))
