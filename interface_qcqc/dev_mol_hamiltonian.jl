...
function dump_ham(ham::Hamiltonian)
    log("\n------dump_ham------- n_qubit: $(ham.n_qubits) n_qubit_ops: $(length(ham.qubit_op))")
...
function get_ham(data_path, _storage_type="BASE")
    op = read_binary_qubit_op(data_path)
    ##########################C############################
    #@ccall couplelib.read_qubit_op(data_path)
    org_ham = MolecularHamiltonian(op)
    new_ham = nothing
    
    st = time_ns()
    storage_type = uppercase(_storage_type)
    if storage_type == "BASE"
        new_ham = org_ham
    elseif storage_type == "INT"
        new_ham = extract_indices_ham_int(org_ham)
    elseif storage_type == "BIT"
        new_ham = extract_indices_ham_bit(org_ham)
    elseif storage_type == "INT_OPT"
        new_ham = extract_indices_ham_int_opt(org_ham)
    else
        error("Unsupport storage type: $storage_type !")
    end
    tot = (time_ns() - st) / 10^9
    log("[Memory] org_ham: $(memsize(org_ham)), new_ham: $(memsize(new_ham)), time: $tot s")
    return org_ham, new_ham
end

memsize(x) = Base.format_bytes(Base.summarysize(x))
...
function _apply_term_on_basis(term::Tuple, coeff::ComplexF64, state::NNQS.ComputationBasis)
    ...
    # println("res_state: $res_state coeff: $(real(res_coeff))")
    return res_state, real(res_coeff)
end

function state2id(state::NNQS.ComputationBasis)
    two = 1
    res = 0
    for i in state
        if i == 1
...
# global_state_cache = Dict{Vector{Int}, Vector{Union{Vector{Int}, Number}}}()
global_state_cache = Dict()

coupled_time = 0.0
# rank = MPI.Comm_rank(MPI.COMM_WORLD)
function coupled_states_gcahce(h::MolecularHamiltonian, state::NNQS.ComputationBasis)
    st = time_ns()
    id = state2id(state)
    global global_state_cache
    if haskey(global_state_cache, id)
        return global_state_cache[id][1], global_state_cache[id][2]
    end
    ...
    global coupled_time += time_ns() - st
    # println("coupled_time: $(coupled_time/10^9) seconds")
    global_state_cache[id] = [c_states, coefs]
...
function coupled_states(h::MolecularHamiltonian, state::NNQS.ComputationBasis)
    ...
    for (term, coeff) in qubit_op
        new_state, new_coeff = _apply_term_on_basis(term, coeff, state)
        cnt += 1
        # println("[ans] $cnt: res_state: $new_state coeff: $(new_coeff) $(state2id(new_state))")
        if new_state in keys(res_dict)
            res_dict[new_state] += new_coeff
        else
            res_dict[new_state] = new_coeff
